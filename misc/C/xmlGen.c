/*
 * Compile using : gcc xmlGen.c -I/usr/include/libxml2 -L/usr/lib -lxml2 -lz -lpthread -lm
 * Run using	 : ./a.out (output to stdout)
 * 			or 
 * 		   ./a.out output.xml (output to file)
 *
 * Note 1	 : The function void create_child(xmlNodePtr root_node, int attr_on) can operate in two modes :
 * 			1. With attribute
 * 			In this mode user cannot add the attribute to the child node.
 * 			The second argument of function is 0 in this case.
 *			2. Without attribute
 *			In this mode user can add the attribute to the child node.
 *			The second argument of function is 1 in this case.
 *
 * Note 2	 : Only child name field can be numeric only. Invalid xml will be generated on adding numeric only entry for other fields.
 *
 */
